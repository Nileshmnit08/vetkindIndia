...
export const metadata = {
  title: "News & Events | VetKind",
  description: "Stay updated with the latest news, announcements, and events from VetKind.",
};
  ...
  return (
    <div className="flex min-h-screen flex-col font-sans bg-zinc-50 dark:bg-zinc-950">
      ...
      <section className="bg-green-900 py-16 text-white md:py-24">
        <div className="container mx-auto px-4 text-center md:px-6">
          ...
          <p className="mx-auto text-xl text-green-100 max-w-2xl leading-relaxed">
            Stay updated with the latest announcements, webinars, and field events from VetKind.
          </p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 py-12 md:px-6 md:py-16">
        ...
        {/* Filters */}
        <div className="mb-8 flex gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-4 overflow-x-auto">
          <Link 
            href="/news-events" 
            className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap ${!typeFilter ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200 dark:bg-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800 transition-colors'}`}
          >
            All Updates
          </Link>
          <Link 
            href="/news-events?type=NEWS"
            className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap ${typeFilter === 'NEWS' ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200 dark:bg-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800 transition-colors'}`}
          >
            Company News
          </Link>
          <Link 
            href="/news-events?type=EVENT"
            className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap ${typeFilter === 'EVENT' ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200 dark:bg-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800 transition-colors'}`}
          >
            Events & Webinars
          </Link>
        </div>
        ...
        <div className="mb-8 flex items-center justify-between text-sm text-zinc-500 dark:text-zinc-400">
          <div className="flex items-center gap-2">
            <span className="font-medium text-zinc-900 dark:text-zinc-100">{items.length} {items.length === 1 ? 'item' : 'items'} found</span>
          </div>
        </div>
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center rounded-3xl border border-dashed border-zinc-300 bg-white dark:border-zinc-800 dark:bg-zinc-900/50">
            ...
            <h2 className="mb-2 text-2xl font-bold text-zinc-900 dark:text-white">
              No {typeFilter === 'EVENT' ? 'events' : typeFilter === 'NEWS' ? 'news' : 'updates'} published yet
            </h2>
            <p className="mb-8 max-w-md text-zinc-500 dark:text-zinc-400">
              We're constantly working on new announcements. Check back soon.
            </p>
            <Link 
              href="/news-events" 
          ...
          <>
            ...
            {featuredItem && (
              <div className="mb-16 overflow-hidden rounded-3xl bg-white shadow-xl shadow-zinc-200/50 dark:bg-zinc-900 dark:shadow-none border border-zinc-100 dark:border-zinc-800 transition-all hover:shadow-2xl">
                <Link href={`/news-events/${featuredItem.slug}`} className="grid lg:grid-cols-2 group">
                  <div className="relative aspect-video lg:aspect-auto bg-zinc-200 dark:bg-zinc-800 overflow-hidden">
                    {featuredItem.coverImage ? (
                      ...
                      <div className="flex h-full w-full flex-col items-center justify-center bg-gradient-to-br from-green-50 to-green-100 text-green-600 dark:from-zinc-800 dark:to-zinc-900 dark:text-green-500 p-12 text-center">
                        <Megaphone className="h-16 w-16 opacity-50 mb-4" />
                        <span className="font-bold opacity-50 tracking-widest uppercase">Featured {featuredItem.type}</span>
                      </div>
                    )}
                  </div>
                  <div className="flex flex-col justify-center p-8 lg:p-12">
                    <span className="mb-4 inline-block w-fit rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-800 dark:bg-green-900/30 dark:text-green-400">
                      {featuredItem.type === 'EVENT' ? 'Event' : 'News'}
                    </span>
                    ...
                    <div className="mt-auto flex flex-col gap-2 text-sm text-zinc-500 border-t border-zinc-100 pt-6 dark:border-zinc-800">
                      {featuredItem.eventDate && (
                        <span className="flex items-center gap-2 text-green-700 dark:text-green-400 font-medium">
                          <Calendar className="h-4 w-4" />
                          Event Date: {new Date(featuredItem.eventDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                        </span>
                      )}
                      ...
                      <span className="flex items-center font-bold text-zinc-900 dark:text-white mt-4">
            ...
            <div className="mb-12">
              {featuredItem && gridItems.length > 0 && (
                <h2 className="mb-8 text-2xl font-bold text-zinc-900 dark:text-white">More Updates</h2>
              )}
              <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                {gridItems.map((item) => (
                  <Link key={item.id} href={`/news-events/${item.slug}`} className="group flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm border border-zinc-200 transition-all hover:-translate-y-1 hover:shadow-xl dark:bg-zinc-900 dark:border-zinc-800">
