import { getBlogArticleBySlug } from "@/app/actions/blog";
import { notFound } from "next/navigation";
import Link from "next/link";
import { ChevronRight, Calendar, User, Share2, FileText } from "lucide-react";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: article.seoTitle || `${article.title} | VetKind Blog`,
      ...
      <div className="bg-white py-4 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex flex-wrap items-center gap-2 text-sm text-zinc-500">
            ...
            {article.category && (
              <>
                <Link href={`/blog?category=${article.category}`} className="hover:text-green-600 transition-colors">{article.category}</Link>
                <ChevronRight className="h-4 w-4" />
              </>
            )}
            ...
            <div className="mt-16 flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-t border-zinc-200 dark:border-zinc-800 pt-8">
              <div className="flex flex-wrap gap-2">
                {article.tags && article.tags.split(',').map((tag, i) => (
                  <span key={i} className="rounded-full bg-zinc-100 px-3 py-1 text-xs font-medium text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">
                    {tag.trim()}
                  </span>
                ))}
              </div>
