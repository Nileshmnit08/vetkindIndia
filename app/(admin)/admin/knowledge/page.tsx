...
import { KnowledgeTableActions } from "@/components/admin/KnowledgeTableActions";
...
export default async function AdminKnowledgePage({ searchParams }: AdminKnowledgePageProps) {
  ...
  const response = await getKnowledgeArticles(query, category, status);
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Manage articles, research, and insights for the Knowledge Centre.
          </p>
        </div>
                    ...
                    articles.map((article) => (
                      <tr key={article.id}>
                        ...
                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                          <KnowledgeTableActions id={article.id} status={article.status} title={article.title} />
                        </td>
                      </tr>
                    ))
                  ) : (
