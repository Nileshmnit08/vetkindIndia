...
import { toggleResearchArticleStatus, deleteResearchArticle } from "@/app/actions/research";
import { useRouter } from "next/navigation";

interface ResearchTableActionsProps {
  id: string;
  status: string;
  title: string;
}

export function ResearchTableActions({ id, status, title }: ResearchTableActionsProps) {
  ...
  const handleToggleStatus = async () => {
    setIsUpdating(true);
    await toggleResearchArticleStatus(id, status);
    setIsUpdating(false);
    setIsOpen(false);
  };
