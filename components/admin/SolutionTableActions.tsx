...
import { toggleSolutionStatus, deleteSolution } from "@/app/actions/solutions";
import { useRouter } from "next/navigation";

interface SolutionTableActionsProps {
  id: string;
  status: string;
  title: string;
}

export function SolutionTableActions({ id, status, title }: SolutionTableActionsProps) {
  ...
  const handleToggleStatus = async () => {
    setIsUpdating(true);
    await toggleSolutionStatus(id, status);
    setIsUpdating(false);
    setIsOpen(false);
  };

  const handleDelete = async () => {
    if (confirm(`Are you sure you want to delete the solution "${title}"? This cannot be undone.`)) {
      setIsUpdating(true);
