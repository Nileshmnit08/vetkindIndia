...
interface NewsEventTableActionsProps {
  id: string;
  status: string;
  title: string;
}

export function NewsEventTableActions({ id, status, title }: NewsEventTableActionsProps) {
  ...
  const handleToggleStatus = async () => {
    setIsUpdating(true);
    await toggleNewsEventStatus(id, status);
    setIsUpdating(false);
    setIsOpen(false);
  };
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 z-20 mt-2 w-48 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-zinc-800 dark:ring-zinc-700">
            <div className="py-1">
              <Link
                href={`/admin/news-events/${id}/edit`}
