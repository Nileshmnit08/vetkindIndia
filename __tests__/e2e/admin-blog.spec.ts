...
test.describe('Admin Blog Page', () => {
  ...
  test('should render the blog page with filters and table', async ({ page }) => {
  ...
  test('should filter items by search query', async ({ page }) => {
    await page.goto('http://localhost:3000/admin/blog');
    await page.fill('input[placeholder*="Search articles"]', 'TestBlogArticleXYZ');
    await page.keyboard.press('Enter');
    
    await page.waitForURL(/q=TestBlogArticleXYZ/);
