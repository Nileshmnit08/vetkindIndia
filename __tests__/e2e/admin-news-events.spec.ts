...
test.describe('Admin News & Events Page', () => {
  ...
  test('should render the news & events page with filters and table', async ({ page }) => {
    await page.goto('http://localhost:3000/admin/news-events');
  ...
  test('should filter items by search query', async ({ page }) => {
    await page.goto('http://localhost:3000/admin/news-events');
    await page.fill('input[placeholder*="Search news"]', 'TestNewsEventXYZ');
    await page.keyboard.press('Enter');
    
    await page.waitForURL(/q=TestNewsEventXYZ/);
    const tableText = await page.locator('table').innerText();
    expect(tableText).toContain('No items found matching your criteria.');
  });
